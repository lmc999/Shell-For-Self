...
start_udpspeeder(){
	...
	nohup speederv2 -s -l0.0.0.0:${port2} -r 127.0.0.1:${port1} -f2:4 --mode 0 --timeout 0 -k passwd >speeder.log 2>&1 &
    if [ "${OS}" == 'CentOS' ];then
        sed -i '/exit/d' /etc/rc.d/rc.local
        echo "nohup speederv2 -s -l0.0.0.0:${port2} -r 127.0.0.1:${port1} -f2:4 --mode 0 --timeout 0 -k passwd >speeder.log 2>&1 &" >> /etc/rc.d/rc.local
	echo "sleep 2
	" >> /etc/rc.d/rc.local
        chmod +x /etc/rc.d/rc.local
    elif [ -s /etc/rc.local ]; then
        sed -i '/exit/d' /etc/rc.local
        echo "nohup speederv2 -s -l0.0.0.0:${port2} -r 127.0.0.1:${port1} -f2:4 --mode 0 --timeout 0 -k passwd >speeder.log 2>&1 &" >> /etc/rc.local
	echo "sleep 2
	" >> /etc/rc.local
